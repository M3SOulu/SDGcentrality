...
output_directory = "." 
model_name = "blog-microservices_0452f7d"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [load_balancer, circuit_breaker, resource_server, local_logging, internal], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/{productId}', '/']"})
...
auth_server = CClass(service, "auth-server", stereotype_instances = [authorization_server, resource_server, ssl_enabled, infrastructural], tagged_values = {'Endpoints': "['/user']", 'Port': 9999, 'Authorization Server': 'Spring OAuth2'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [load_balancer, resource_server, local_logging, infrastructural, gateway], tagged_values = {'Gateway': 'Zuul', 'Port': 8765, 'Load Balancer': 'Ribbon'})
