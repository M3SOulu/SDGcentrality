...
output_directory = "." 
model_name = "blog-microservices_06eeb7f"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [internal, load_balancer, local_logging, circuit_breaker, resource_server], tagged_values = {'Endpoints': "['/{productId}', '/']", 'Load Balancer': 'Spring Cloud'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [gateway, load_balancer, infrastructural, local_logging, resource_server], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
