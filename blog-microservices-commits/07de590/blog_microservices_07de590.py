...
output_directory = "." 
model_name = "blog-microservices_07de590"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [load_balancer, resource_server, circuit_breaker, local_logging, internal], tagged_values = {'Endpoints': "['/{productId}', '/']", 'Load Balancer': 'Spring Cloud'})
...
auth_server = CClass(service, "auth-server", stereotype_instances = [infrastructural, authorization_server, resource_server], tagged_values = {'Endpoints': "['/user']", 'Authorization Server': 'Spring OAuth2'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [load_balancer, resource_server, gateway, infrastructural, local_logging], tagged_values = {'Gateway': 'Zuul', 'Port': 8765, 'Load Balancer': 'Ribbon'})
