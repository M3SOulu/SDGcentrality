...
composite_service = CClass(service, "composite-service", stereotype_instances = [load_balancer, local_logging, circuit_breaker, internal, resource_server], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/', '/{productId}']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [load_balancer, local_logging, infrastructural, gateway, resource_server], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
