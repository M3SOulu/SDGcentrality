...
output_directory = "." 
model_name = "blog-microservices_16c9cc9"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [internal, resource_server, circuit_breaker, local_logging], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/{productId}', '/']"})
hystrix_dashboard = CClass(service, "hystrix-dashboard", stereotype_instances = [monitoring_dashboard, infrastructural], tagged_values = {'Port': 7979, 'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']"})
configserver = CClass(service, "configserver", stereotype_instances = [configuration_server, infrastructural, local_logging], tagged_values = {'Configuration Server': 'Spring Cloud Config', 'Port': 8888})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul', 'Port': 8765})
