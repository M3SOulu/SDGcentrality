...
output_directory = "." 
model_name = "blog-microservices_1857bca"
oauth2_vanilla_ui = CClass(service, "oauth2-vanilla-ui", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [infrastructural, local_logging, monitoring_dashboard], tagged_values = {'Endpoints': "['/']", 'Port': 7979, 'Monitoring Dashboard': 'Hystrix'})
oauth2_vanilla_authserver = CClass(service, "oauth2-vanilla-authserver", stereotype_instances = [authorization_server, resource_server, infrastructural], tagged_values = {'Endpoints': "['/user']", 'Port': 9999, 'Authorization Server': 'Spring OAuth2'})
...
zuulserver = CClass(service, "zuulserver", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Port': 8765, 'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
products = CClass(service, "products", stereotype_instances = [circuit_breaker, internal], tagged_values = {'Endpoints': "['/products/{productId}']", 'Port': 10011, 'Circuit Breaker': 'Hystrix'})
reviews = CClass(service, "reviews", stereotype_instances = [circuit_breaker, internal], tagged_values = {'Port': 10002, 'Endpoints': "['/reviews']", 'Circuit Breaker': 'Hystrix'})
productapi = CClass(service, "productapi", stereotype_instances = [local_logging, circuit_breaker, internal], tagged_values = {'Endpoints': "['/products/{productId}', '/']", 'Port': 10021, 'Circuit Breaker': 'Hystrix'})
...
add_links({products: eureka}, stereotype_instances = [restful_http, circuit_breaker_link])
add_links({reviews: eureka}, stereotype_instances = [restful_http, circuit_breaker_link])
