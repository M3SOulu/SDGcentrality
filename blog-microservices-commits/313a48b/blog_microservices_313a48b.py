...
output_directory = "." 
model_name = "blog-microservices_313a48b"
...
product_composite = CClass(service, "product-composite", stereotype_instances = [local_logging, circuit_breaker, internal], tagged_values = {'Endpoints': "['/', '/product/{productId}']", 'Port': 8080, 'Circuit Breaker': 'Hystrix'})
...
configserver = CClass(service, "configserver", stereotype_instances = [configuration_server, local_logging, infrastructural], tagged_values = {'Configuration Server': 'Spring Cloud Config', 'Port': 8888})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Port': 8765, 'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
productapi = CClass(service, "productapi", stereotype_instances = [local_logging, circuit_breaker, internal, resource_server], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/{productId}']"})
