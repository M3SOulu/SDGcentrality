...
output_directory = "." 
model_name = "blog-microservices_440d8dc"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [resource_server, circuit_breaker, local_logging, internal], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/', '/{productId}']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon', 'Port': 8765})
...
productapi = CClass(service, "productapi", stereotype_instances = [resource_server, circuit_breaker, local_logging, internal], tagged_values = {'Endpoints': "['/{productId}']", 'Circuit Breaker': 'Hystrix'})
