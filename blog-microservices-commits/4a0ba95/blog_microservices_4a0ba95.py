...
output_directory = "." 
model_name = "blog-microservices_4a0ba95"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [local_logging, circuit_breaker, load_balancer, resource_server, internal], tagged_values = {'Endpoints': "['/', '/{productId}']", 'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [infrastructural, local_logging, gateway, load_balancer, resource_server], tagged_values = {'Port': 8765, 'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
...
add_links({composite_service: eureka}, stereotype_instances = [circuit_breaker_link, restful_http, load_balanced_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
