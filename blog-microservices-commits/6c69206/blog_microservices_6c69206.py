...
output_directory = "." 
model_name = "blog-microservices_6c69206"
...
review_service = CClass(service, "review-service", stereotype_instances = [local_logging, internal], tagged_values = {'Endpoints': "['/set-processing-time', '/review']", 'Port': 8080})
composite_service = CClass(service, "composite-service", stereotype_instances = [local_logging, circuit_breaker, resource_server, internal], tagged_values = {'Endpoints': "['/', '/{productId}']", 'Circuit Breaker': 'Hystrix'})
...
configserver = CClass(service, "configserver", stereotype_instances = [local_logging, configuration_server, infrastructural], tagged_values = {'Port': 8888, 'Configuration Server': 'Spring Cloud Config'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [gateway, load_balancer, infrastructural], tagged_values = {'Gateway': 'Zuul', 'Port': 8765, 'Load Balancer': 'Ribbon'})
...
productapi = CClass(service, "productapi", stereotype_instances = [local_logging, circuit_breaker, resource_server, internal], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/{productId}']"})
