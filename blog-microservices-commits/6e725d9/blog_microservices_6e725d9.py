...
output_directory = "." 
model_name = "blog-microservices_6e725d9"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [circuit_breaker, local_logging, internal, resource_server, load_balancer], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/{productId}', '/']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [gateway, load_balancer, local_logging, resource_server, infrastructural], tagged_values = {'Port': 8765, 'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
add_links({composite_service: eureka}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud'})
