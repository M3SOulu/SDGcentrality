...
output_directory = "." 
model_name = "blog-microservices_7cccdd5"
webapp = CClass(service, "webapp", stereotype_instances = [infrastructural, load_balancer, gateway], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [circuit_breaker, internal, local_logging], tagged_values = {'Endpoints': "['/product/{productId}', '/']", 'Port': 8080, 'Circuit Breaker': 'Hystrix'})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [infrastructural, monitoring_dashboard], tagged_values = {'Monitoring Dashboard': 'Hystrix', 'Port': 7979, 'Endpoints': "['/']"})
...
turbine = CClass(service, "turbine", stereotype_instances = [infrastructural, monitoring_server, local_logging], tagged_values = {'Monitoring Server': 'Turbine', 'Port': 8989})
edgeserver = CClass(service, "edgeserver", stereotype_instances = [infrastructural, load_balancer, gateway], tagged_values = {'Port': 8765, 'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
