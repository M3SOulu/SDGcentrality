...
output_directory = "." 
model_name = "blog-microservices_7e20025"
...
recommendation = CClass(service, "recommendation", stereotype_instances = [internal, local_logging], tagged_values = {'Endpoints': "['/set-processing-time', '/recommendation']"})
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [internal, circuit_breaker, local_logging], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/', '/product/{productId}']"})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [infrastructural, monitoring_dashboard, local_logging], tagged_values = {'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']", 'Port': 7979})
...
zuulserver = CClass(service, "zuulserver", stereotype_instances = [infrastructural, load_balancer, gateway], tagged_values = {'Load Balancer': 'Ribbon', 'Port': 8765, 'Gateway': 'Zuul'})
...
productapi = CClass(service, "productapi", stereotype_instances = [internal, circuit_breaker, local_logging], tagged_values = {'Endpoints': "['/ping', '/product/{productId}']", 'Port': 10001, 'Circuit Breaker': 'Hystrix'})
