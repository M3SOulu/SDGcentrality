...
output_directory = "." 
model_name = "blog-microservices_8028a1b"
webapp = CClass(service, "webapp", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
product = CClass(service, "product", stereotype_instances = [local_logging, internal], tagged_values = {'Endpoints': "['/product/{productId}']", 'Port': 8080})
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [local_logging, internal, circuit_breaker], tagged_values = {'Endpoints': "['/product/{productId}', '/']", 'Port': 8080, 'Circuit Breaker': 'Hystrix'})
...
edgeserver = CClass(service, "edgeserver", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Port': 8765, 'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
productapi = CClass(service, "productapi", stereotype_instances = [local_logging, internal, circuit_breaker], tagged_values = {'Endpoints': "['/{productId}']", 'Port': 8080, 'Circuit Breaker': 'Hystrix'})
