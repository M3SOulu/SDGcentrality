...
output_directory = "." 
model_name = "blog-microservices_871c903"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [circuit_breaker, resource_server, internal, local_logging], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/', '/{productId}']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul', 'Port': 8765})
...
productapi = CClass(service, "productapi", stereotype_instances = [circuit_breaker, resource_server, internal, local_logging], tagged_values = {'Endpoints': "['/{productId}']", 'Circuit Breaker': 'Hystrix'})
