...
output_directory = "." 
model_name = "blog-microservices_a223c00"
webapp = CClass(service, "webapp", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
...
product = CClass(service, "product", stereotype_instances = [internal], tagged_values = {'Port': 8080, 'Endpoints': "['/product/{productId}']"})
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [internal, local_logging, circuit_breaker], tagged_values = {'Port': 8080, 'Endpoints': "['/', '/product/{productId}']"})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [monitoring_dashboard, infrastructural], tagged_values = {'Port': 7979, 'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']"})
...
turbine = CClass(service, "turbine", stereotype_instances = [monitoring_server, infrastructural, local_logging], tagged_values = {'Monitoring Server': 'Turbine', 'Port': 8989})
...
productapi = CClass(service, "productapi", stereotype_instances = [internal, local_logging, circuit_breaker], tagged_values = {'Port': 8080, 'Endpoints': "['/{productId}']"})
