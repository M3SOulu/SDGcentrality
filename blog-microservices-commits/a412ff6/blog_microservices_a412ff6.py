...
output_directory = "." 
model_name = "blog-microservices_a412ff6"
recommendation = CClass(service, "recommendation", stereotype_instances = [internal, local_logging], tagged_values = {'Port': 8080, 'Endpoints': "['/set-processing-time', '/recommendation']"})
product = CClass(service, "product", stereotype_instances = [internal, local_logging], tagged_values = {'Port': 8080, 'Endpoints': "['/set-processing-time', '/product/{productId}']"})
review = CClass(service, "review", stereotype_instances = [internal, local_logging], tagged_values = {'Port': 8080, 'Endpoints': "['/set-processing-time', '/review']"})
productcomposite = CClass(service, "productcomposite", stereotype_instances = [internal, circuit_breaker, local_logging], tagged_values = {'Port': 8080, 'Circuit Breaker': 'Hystrix', 'Endpoints': "['/product/{productId}', '/']"})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [infrastructural, monitoring_dashboard], tagged_values = {'Port': 7979, 'Endpoints': "['/']", 'Monitoring Dashboard': 'Hystrix'})
configserver = CClass(service, "configserver", stereotype_instances = [configuration_server, infrastructural, local_logging], tagged_values = {'Port': 8888, 'Configuration Server': 'Spring Cloud Config'})
turbine = CClass(service, "turbine", stereotype_instances = [infrastructural, local_logging, monitoring_server], tagged_values = {'Monitoring Server': 'Turbine', 'Port': 8989})
edgeserver = CClass(service, "edgeserver", stereotype_instances = [infrastructural, load_balancer, gateway], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul', 'Port': 8765})
...
productapi = CClass(service, "productapi", stereotype_instances = [internal, resource_server, circuit_breaker, local_logging], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/{productId}']"})
