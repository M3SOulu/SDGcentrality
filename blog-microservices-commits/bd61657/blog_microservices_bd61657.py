...
output_directory = "." 
model_name = "blog-microservices_bd61657"
...
recommendation = CClass(service, "recommendation", stereotype_instances = [local_logging, internal], tagged_values = {'Endpoints': "['/recommendation', '/set-processing-time']"})
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [local_logging, internal, circuit_breaker], tagged_values = {'Endpoints': "['/product/{productId}', '/']", 'Circuit Breaker': 'Hystrix'})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [monitoring_dashboard, local_logging, infrastructural], tagged_values = {'Endpoints': "['/']", 'Port': 7979, 'Monitoring Dashboard': 'Hystrix'})
...
zuulserver = CClass(service, "zuulserver", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon', 'Port': 8765})
...
productapi = CClass(service, "productapi", stereotype_instances = [local_logging, internal, circuit_breaker], tagged_values = {'Endpoints': "['/product/{productId}', '/ping']", 'Port': 10001, 'Circuit Breaker': 'Hystrix'})
