...
output_directory = "." 
model_name = "blog-microservices_d6f1905"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [resource_server, internal, circuit_breaker, local_logging, load_balancer], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/', '/{productId}']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [infrastructural, resource_server, local_logging, gateway, load_balancer], tagged_values = {'Load Balancer': 'Ribbon', 'Port': 8765, 'Gateway': 'Zuul'})
