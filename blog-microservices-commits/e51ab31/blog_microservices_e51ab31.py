...
output_directory = "." 
model_name = "blog-microservices_e51ab31"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [load_balancer, resource_server, circuit_breaker, internal, local_logging], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/{productId}', '/']"})
...
auth_server = CClass(service, "auth-server", stereotype_instances = [authorization_server, infrastructural, resource_server], tagged_values = {'Authorization Server': 'Spring OAuth2', 'Endpoints': "['/user']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [gateway, infrastructural, load_balancer, resource_server, local_logging], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
