...
output_directory = "." 
model_name = "blog-microservices_f36be8e"
oauth2_vanilla_ui = CClass(service, "oauth2-vanilla-ui", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [monitoring_dashboard, local_logging, infrastructural], tagged_values = {'Port': 7979, 'Endpoints': "['/']", 'Monitoring Dashboard': 'Hystrix'})
...
zuulserver = CClass(service, "zuulserver", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon', 'Port': 8765})
...
products = CClass(service, "products", stereotype_instances = [internal], tagged_values = {'Port': 10011, 'Endpoints': "['/products/{productId}']"})
reviews = CClass(service, "reviews", stereotype_instances = [local_logging, internal], tagged_values = {'Endpoints': "['/set-processing-time', '/reviews']"})
...
productapi = CClass(service, "productapi", stereotype_instances = [local_logging, internal, circuit_breaker], tagged_values = {'Circuit Breaker': 'Hystrix', 'Port': 10021, 'Endpoints': "['/products/{productId}', '/']"})
