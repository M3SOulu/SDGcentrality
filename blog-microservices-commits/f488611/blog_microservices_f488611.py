...
output_directory = "." 
model_name = "blog-microservices_f488611"
...
product_service = CClass(service, "product-service", stereotype_instances = [internal, local_logging], tagged_values = {'Port': 8080, 'Endpoints': "['/set-processing-time', '/product/{productId}']"})
...
composite_service = CClass(service, "composite-service", stereotype_instances = [internal, resource_server, local_logging, circuit_breaker], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/{productId}', '/']"})
hystrix_dashboard = CClass(service, "hystrix-dashboard", stereotype_instances = [monitoring_dashboard, infrastructural], tagged_values = {'Monitoring Dashboard': 'Hystrix', 'Port': 7979, 'Endpoints': "['/']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Load Balancer': 'Ribbon', 'Port': 8765, 'Gateway': 'Zuul'})
...
add_links({config_server: productapi}, stereotype_instances = [restful_http])
...
add_links({composite_service: eureka}, stereotype_instances = [circuit_breaker_link, restful_http], tagged_values = {'Circuit Breaker': 'Hystrix'})
