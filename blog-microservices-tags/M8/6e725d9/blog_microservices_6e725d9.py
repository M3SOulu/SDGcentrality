...
composite_service = CClass(service, "composite-service", stereotype_instances = [local_logging, load_balancer, internal, resource_server, circuit_breaker], tagged_values = {'Endpoints': "['/{productId}', '/']", 'Load Balancer': 'Spring Cloud'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [infrastructural, gateway, local_logging, load_balancer, resource_server], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon', 'Port': 8765})
