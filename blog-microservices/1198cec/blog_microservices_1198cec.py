...
output_directory = "." 
model_name = "blog-microservices_1198cec"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [local_logging, internal, resource_server, load_balancer, circuit_breaker], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/', '/{productId}']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [gateway, local_logging, resource_server, infrastructural, load_balancer], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
