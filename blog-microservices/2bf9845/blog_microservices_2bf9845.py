...
output_directory = "." 
model_name = "blog-microservices_2bf9845"
oauth2_vanilla_ui = CClass(service, "oauth2-vanilla-ui", stereotype_instances = [infrastructural, load_balancer, gateway], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [local_logging, monitoring_dashboard, infrastructural], tagged_values = {'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']", 'Port': 7979})
zuulserver = CClass(service, "zuulserver", stereotype_instances = [infrastructural, load_balancer, gateway], tagged_values = {'Gateway': 'Zuul', 'Port': 8765, 'Load Balancer': 'Ribbon'})
...
products = CClass(service, "products", stereotype_instances = [circuit_breaker, internal], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/products/{productId}']", 'Port': 10011})
reviews = CClass(service, "reviews", stereotype_instances = [circuit_breaker, internal], tagged_values = {'Circuit Breaker': 'Hystrix', 'Port': 10002, 'Endpoints': "['/reviews']"})
...
productapi = CClass(service, "productapi", stereotype_instances = [local_logging, circuit_breaker, internal], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/', '/products/{productId}']", 'Port': 10021})
