...
output_directory = "." 
model_name = "blog-microservices_3646f65"
recommendation_service = CClass(service, "recommendation-service", stereotype_instances = [local_logging, internal], tagged_values = {'Port': 8080, 'Endpoints': "['/set-processing-time', '/recommendation']"})
...
composite_service = CClass(service, "composite-service", stereotype_instances = [resource_server, circuit_breaker, internal, local_logging, load_balancer], tagged_values = {'Endpoints': "['/{productId}', '/']", 'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [local_logging, resource_server, infrastructural, gateway, load_balancer], tagged_values = {'Gateway': 'Zuul', 'Port': 8765, 'Load Balancer': 'Ribbon'})
...
add_links({composite_service: eureka}, stereotype_instances = [circuit_breaker_link, restful_http, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
