...
output_directory = "." 
model_name = "blog-microservices_384c739"
oauth2_vanilla_ui = CClass(service, "oauth2-vanilla-ui", stereotype_instances = [load_balancer, gateway, infrastructural], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [monitoring_dashboard, local_logging, infrastructural], tagged_values = {'Port': 7979, 'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']"})
zuulserver = CClass(service, "zuulserver", stereotype_instances = [load_balancer, gateway, infrastructural], tagged_values = {'Load Balancer': 'Ribbon', 'Port': 8765, 'Gateway': 'Zuul'})
...
productapi = CClass(service, "productapi", stereotype_instances = [circuit_breaker, local_logging, internal], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/products/{productId}', '/']", 'Port': 10021})
