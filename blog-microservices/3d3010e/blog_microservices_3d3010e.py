...
output_directory = "." 
model_name = "blog-microservices_3d3010e"
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [internal, circuit_breaker, local_logging], tagged_values = {'Endpoints': "['/', '/product/{productId}']"})
...
edgeserver = CClass(service, "edgeserver", stereotype_instances = [gateway, authentication_scope_all_requests, load_balancer, infrastructural, local_logging, basic_authentication], tagged_values = {'Port': 8765, 'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
productapi = CClass(service, "productapi", stereotype_instances = [internal, circuit_breaker, local_logging], tagged_values = {'Endpoints': "['/{productId}']"})
