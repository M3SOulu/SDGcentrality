...
output_directory = "." 
model_name = "blog-microservices_5b7c69f"
...
product = CClass(service, "product", stereotype_instances = [internal, local_logging], tagged_values = {'Endpoints': "['/set-processing-time', '/product/{productId}']", 'Port': 8080})
review = CClass(service, "review", stereotype_instances = [internal, local_logging], tagged_values = {'Endpoints': "['/set-processing-time', '/review']", 'Port': 8080})
...
turbine = CClass(service, "turbine", stereotype_instances = [infrastructural, monitoring_server, local_logging], tagged_values = {'Port': 8989, 'Monitoring Server': 'Turbine'})
edgeserver = CClass(service, "edgeserver", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Gateway': 'Zuul', 'Port': 8765, 'Load Balancer': 'Ribbon'})
...
productapi = CClass(service, "productapi", stereotype_instances = [internal, resource_server, local_logging, circuit_breaker], tagged_values = {'Endpoints': "['/{productId}']", 'Circuit Breaker': 'Hystrix'})
...
github_repository = CClass(external_component, "github-repository", stereotype_instances = [github_repository, entrypoint], tagged_values = {'URL': 'https://github.com/callistaenterprise/blog-microservices-config.git\n'})
