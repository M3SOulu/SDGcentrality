...
output_directory = "." 
model_name = "blog-microservices_671f8bf"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [internal, resource_server, load_balancer, local_logging, circuit_breaker], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/{productId}', '/']"})
monitor_dashboard = CClass(service, "monitor-dashboard", stereotype_instances = [local_logging, monitoring_dashboard, infrastructural], tagged_values = {'Endpoints': "['/']", 'Monitoring Dashboard': 'Hystrix'})
auth_server = CClass(service, "auth-server", stereotype_instances = [authorization_server, local_logging, infrastructural, resource_server], tagged_values = {'Authorization Server': 'Spring OAuth2', 'Endpoints': "['/user']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [gateway, load_balancer, local_logging, infrastructural, resource_server], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
