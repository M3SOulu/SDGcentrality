...
output_directory = "." 
model_name = "blog-microservices_7834b74"
webapp = CClass(service, "webapp", stereotype_instances = [infrastructural, load_balancer, gateway], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
recommendation = CClass(service, "recommendation", stereotype_instances = [internal, local_logging], tagged_values = {'Endpoints': "['/set-processing-time', '/recommendation']", 'Port': 8080})
product = CClass(service, "product", stereotype_instances = [internal, local_logging], tagged_values = {'Port': 8080, 'Endpoints': "['/product/{productId}']"})
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [circuit_breaker, local_logging, internal], tagged_values = {'Port': 8080, 'Circuit Breaker': 'Hystrix', 'Endpoints': "['/', '/product/{productId}']"})
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [infrastructural, monitoring_dashboard], tagged_values = {'Port': 7979, 'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']"})
...
turbine = CClass(service, "turbine", stereotype_instances = [monitoring_server, local_logging, infrastructural], tagged_values = {'Monitoring Server': 'Turbine', 'Port': 8989})
...
productapi = CClass(service, "productapi", stereotype_instances = [circuit_breaker, local_logging, internal], tagged_values = {'Endpoints': "['/{productId}']", 'Port': 8080, 'Circuit Breaker': 'Hystrix'})
