...
output_directory = "." 
model_name = "blog-microservices_978fb16"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [resource_server, load_balancer, local_logging, internal, circuit_breaker], tagged_values = {'Load Balancer': 'Spring Cloud', 'Endpoints': "['/', '/{productId}']"})
monitor_dashboard = CClass(service, "monitor-dashboard", stereotype_instances = [monitoring_dashboard, infrastructural], tagged_values = {'Endpoints': "['/']", 'Monitoring Dashboard': 'Hystrix'})
auth_server = CClass(service, "auth-server", stereotype_instances = [authorization_server, resource_server, infrastructural], tagged_values = {'Endpoints': "['/user']", 'Authorization Server': 'Spring OAuth2'})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [resource_server, load_balancer, local_logging, gateway, infrastructural], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
