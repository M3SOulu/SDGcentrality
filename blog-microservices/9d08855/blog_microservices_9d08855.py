...
output_directory = "." 
model_name = "blog-microservices_9d08855"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [internal, load_balancer, circuit_breaker, resource_server, local_logging], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud', 'Endpoints': "['/', '/{productId}']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [gateway, infrastructural, load_balancer, resource_server, local_logging], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul', 'Port': 8765})
...
add_links({composite_service: eureka}, stereotype_instances = [load_balanced_link, restful_http, circuit_breaker_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
