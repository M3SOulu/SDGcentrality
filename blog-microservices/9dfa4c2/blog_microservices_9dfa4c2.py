...
output_directory = "." 
model_name = "blog-microservices_9dfa4c2"
...
composite_service = CClass(service, "composite-service", stereotype_instances = [circuit_breaker, load_balancer, local_logging, internal, resource_server], tagged_values = {'Endpoints': "['/{productId}', '/']", 'Load Balancer': 'Spring Cloud'})
monitor_dashboard = CClass(service, "monitor-dashboard", stereotype_instances = [monitoring_dashboard, infrastructural, local_logging], tagged_values = {'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']"})
...
edge_server = CClass(service, "edge-server", stereotype_instances = [load_balancer, local_logging, gateway, resource_server, infrastructural], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
