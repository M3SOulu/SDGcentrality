...
output_directory = "." 
model_name = "blog-microservices_bed81a7"
...
hystrixdashboard = CClass(service, "hystrixdashboard", stereotype_instances = [monitoring_dashboard, infrastructural, local_logging], tagged_values = {'Monitoring Dashboard': 'Hystrix', 'Port': 7979, 'Endpoints': "['/']"})
...
zuulserver = CClass(service, "zuulserver", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Port': 8765, 'Gateway': 'Zuul', 'Load Balancer': 'Ribbon'})
...
reviews = CClass(service, "reviews", stereotype_instances = [internal, local_logging], tagged_values = {'Endpoints': "['/reviews', '/set-processing-time']"})
productcomposite = CClass(service, "productcomposite", stereotype_instances = [internal, local_logging, circuit_breaker], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/products/{productId}', '/']"})
...
productapi = CClass(service, "productapi", stereotype_instances = [internal, local_logging, circuit_breaker], tagged_values = {'Endpoints': "['/products/{productId}', '/ping']", 'Port': 10021, 'Circuit Breaker': 'Hystrix'})
