...
output_directory = "." 
model_name = "blog-microservices_dfa933e"
recommendation = CClass(service, "recommendation", stereotype_instances = [internal, local_logging], tagged_values = {'Endpoints': "['/recommendation', '/set-processing-time']", 'Port': 8080})
product = CClass(service, "product", stereotype_instances = [internal, local_logging], tagged_values = {'Port': 8080, 'Endpoints': "['/product/{productId}', '/set-processing-time']"})
...
productcomposite = CClass(service, "productcomposite", stereotype_instances = [internal, local_logging, circuit_breaker], tagged_values = {'Circuit Breaker': 'Hystrix', 'Port': 8080, 'Endpoints': "['/product/{productId}', '/']"})
...
configserver = CClass(service, "configserver", stereotype_instances = [local_logging, infrastructural, configuration_server], tagged_values = {'Configuration Server': 'Spring Cloud Config', 'Port': 8888})
turbine = CClass(service, "turbine", stereotype_instances = [local_logging, monitoring_server, infrastructural], tagged_values = {'Port': 8989, 'Monitoring Server': 'Turbine'})
edgeserver = CClass(service, "edgeserver", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Load Balancer': 'Ribbon', 'Port': 8765, 'Gateway': 'Zuul'})
...
productapi = CClass(service, "productapi", stereotype_instances = [internal, local_logging, resource_server, circuit_breaker], tagged_values = {'Endpoints': "['/{productId}']", 'Circuit Breaker': 'Hystrix'})
