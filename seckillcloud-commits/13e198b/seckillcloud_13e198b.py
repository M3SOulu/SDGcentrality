...
output_directory = "." 
model_name = "seckillcloud_13e198b"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [gateway, infrastructural, local_logging], tagged_values = {'Port': 8205, 'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [csrf_disabled, pre_authorized_endpoints, encryption, local_logging, internal], tagged_values = {'Port': 8070, 'Pre-authorized Endpoints': "['/{id}', '/updateUsing/{id}', '/deletes', '/{id}/edit']", 'Logging Technology': 'Lombok'})
cloud_monitor = CClass(service, "cloud-monitor", stereotype_instances = [administration_server, basic_authentication, infrastructural], tagged_values = {'Port': 8890, 'Administration Server': 'Spring Boot Admin'})
mysql = CClass(service, "mysql", stereotype_instances = [internal], tagged_values = {'Port': 3306})
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [plaintext_credentials, infrastructural, message_broker], tagged_values = {'Message Broker': 'RabbitMQ', 'Port': 5672, 'Password': 'guest', 'Username': 'guest'})
