...
output_directory = "." 
model_name = "seckillcloud_18ecf9f"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [infrastructural, local_logging, gateway], tagged_values = {'Port': 8205, 'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [local_logging, encryption, csrf_disabled, internal, pre_authorized_endpoints], tagged_values = {'Pre-authorized Endpoints': "['/getMenus']", 'Logging Technology': 'Lombok', 'Port': 8080})
cloud_monitor = CClass(service, "cloud-monitor", stereotype_instances = [infrastructural, administration_server], tagged_values = {'Administration Server': 'Spring Boot Admin', 'Port': 8890})
database_cloud_uaa = CClass(external_component, "database-cloud-uaa", stereotype_instances = [entrypoint, exitpoint, external_database], tagged_values = {'Port': 3306, 'Database': 'MySQL'})
database_cloud_mission = CClass(external_component, "database-cloud-mission", stereotype_instances = [entrypoint, exitpoint, external_database], tagged_values = {'Port': 3306, 'Database': 'MySQL'})
