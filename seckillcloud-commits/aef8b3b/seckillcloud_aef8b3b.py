...
output_directory = "." 
model_name = "seckillcloud_aef8b3b"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [infrastructural, local_logging, gateway], tagged_values = {'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway', 'Port': 8205})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [internal, encryption, local_logging, pre_authorized_endpoints, csrf_disabled], tagged_values = {'Pre-authorized Endpoints': "['/{id}', '/updateUsing/{id}', '/deletes', '/{id}/edit']", 'Logging Technology': 'Lombok', 'Port': 8070})
cloud_monitor = CClass(service, "cloud-monitor", stereotype_instances = [basic_authentication, infrastructural, administration_server], tagged_values = {'Administration Server': 'Spring Boot Admin', 'Port': 8890})
mysql = CClass(service, "mysql", stereotype_instances = [internal], tagged_values = {'Port': 3305})
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [infrastructural, plaintext_credentials, message_broker], tagged_values = {'Username': 'guest', 'Port': 5672, 'Message Broker': 'RabbitMQ', 'Password': 'guest'})
