...
output_directory = "." 
model_name = "seckillcloud_b958ccf"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [gateway, local_logging, infrastructural], tagged_values = {'Gateway': 'Spring Cloud Gateway', 'Port': 8205, 'Logging Technology': 'Lombok'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [local_logging, internal, csrf_disabled, pre_authorized_endpoints, encryption], tagged_values = {'Port': 8070, 'Logging Technology': 'Lombok', 'Pre-authorized Endpoints': "['/findAll']"})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [plaintext_credentials, message_broker, infrastructural], tagged_values = {'Port': 5672, 'Password': 'guest', 'Message Broker': 'RabbitMQ', 'Username': 'guest'})
