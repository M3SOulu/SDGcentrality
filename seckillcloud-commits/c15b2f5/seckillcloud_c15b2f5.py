...
output_directory = "." 
model_name = "seckillcloud_c15b2f5"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [gateway, local_logging, infrastructural], tagged_values = {'Port': 8205, 'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [csrf_disabled, internal, local_logging, pre_authorized_endpoints, encryption], tagged_values = {'Pre-authorized Endpoints': "['/findAll']", 'Logging Technology': 'Lombok', 'Port': 8070})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [message_broker, plaintext_credentials, infrastructural], tagged_values = {'Username': 'guest', 'Password': 'guest', 'Message Broker': 'RabbitMQ', 'Port': 5672})
