...
output_directory = "." 
model_name = "seckillcloud_d248305"
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [encryption, csrf_disabled, internal, local_logging, pre_authorized_endpoints], tagged_values = {'Logging Technology': 'Lombok', 'Pre-authorized Endpoints': "['/{id}', '/updateUsing/{id}', '/deletes', '/{id}/edit']", 'Port': 8070})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [message_broker, plaintext_credentials, infrastructural], tagged_values = {'Username': 'guest', 'Port': 5672, 'Password': 'guest', 'Message Broker': 'RabbitMQ'})
