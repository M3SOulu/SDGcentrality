...
output_directory = "." 
model_name = "seckillcloud_eddbc3a"
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [pre_authorized_endpoints, internal, encryption, local_logging, csrf_disabled], tagged_values = {'Pre-authorized Endpoints': "['/getMenus']", 'Port': 8070, 'Logging Technology': 'Lombok'})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [infrastructural, message_broker, plaintext_credentials], tagged_values = {'Password': 'guest', 'Username': 'guest', 'Message Broker': 'RabbitMQ', 'Port': 5672})
...
add_links({cloud_mission: rabbitmq}, stereotype_instances = [plaintext_credentials_link, message_producer_rabbitmq, restful_http], tagged_values = {'Producer Exchange': 'None', 'Routing Key': 'None'})
