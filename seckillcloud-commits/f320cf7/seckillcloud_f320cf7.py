...
output_directory = "." 
model_name = "seckillcloud_f320cf7"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [local_logging, gateway, infrastructural], tagged_values = {'Gateway': 'Spring Cloud Gateway', 'Logging Technology': 'Lombok', 'Port': 8205})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [csrf_disabled, local_logging, pre_authorized_endpoints, internal, encryption], tagged_values = {'Pre-authorized Endpoints': "['/{id}/edit', '/{id}', '/deletes', '/updateUsing/{id}']", 'Port': 8070, 'Logging Technology': 'Lombok'})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [plaintext_credentials, infrastructural, message_broker], tagged_values = {'Password': 'guest', 'Message Broker': 'RabbitMQ', 'Username': 'guest', 'Port': 5672})
