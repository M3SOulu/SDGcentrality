...
output_directory = "." 
model_name = "seckillcloud_f5a009a"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [infrastructural, local_logging, gateway], tagged_values = {'Logging Technology': 'Lombok', 'Port': 8205, 'Gateway': 'Spring Cloud Gateway'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [local_logging, csrf_disabled, encryption, internal, pre_authorized_endpoints], tagged_values = {'Pre-authorized Endpoints': "['/getMenus']", 'Logging Technology': 'Lombok', 'Port': 8070})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [infrastructural, plaintext_credentials, message_broker], tagged_values = {'Password': 'guest', 'Port': 5672, 'Message Broker': 'RabbitMQ', 'Username': 'guest'})
