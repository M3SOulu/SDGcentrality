...
output_directory = "." 
model_name = "seckillcloud_1c2dd26"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [infrastructural, gateway, local_logging], tagged_values = {'Gateway': 'Spring Cloud Gateway', 'Port': 8205, 'Logging Technology': 'Lombok'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [csrf_disabled, local_logging, internal, encryption, pre_authorized_endpoints], tagged_values = {'Pre-authorized Endpoints': "['/updateUsing/{id}', '/deletes', '/{id}/edit', '/{id}']", 'Port': 8070, 'Logging Technology': 'Lombok'})
cloud_monitor = CClass(service, "cloud-monitor", stereotype_instances = [infrastructural, administration_server, basic_authentication], tagged_values = {'Port': 8890, 'Administration Server': 'Spring Boot Admin'})
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [infrastructural, message_broker, plaintext_credentials], tagged_values = {'Port': 5672, 'Password': 'guest', 'Message Broker': 'RabbitMQ', 'Username': 'guest'})
...
add_links({cloud_mission: rabbitmq}, stereotype_instances = [plaintext_credentials_link, message_producer_rabbitmq, restful_http], tagged_values = {'Routing Key': 'None', 'Producer Exchange': 'None'})
