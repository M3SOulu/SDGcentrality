...
output_directory = "." 
model_name = "seckillcloud_2814600"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [gateway, infrastructural, local_logging], tagged_values = {'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway', 'Port': 8205})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [pre_authorized_endpoints, csrf_disabled, encryption, local_logging, internal], tagged_values = {'Logging Technology': 'Lombok', 'Pre-authorized Endpoints': "['/{id}']", 'Port': 8070})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [plaintext_credentials, message_broker, infrastructural], tagged_values = {'Password': 'guest', 'Port': 5672, 'Message Broker': 'RabbitMQ', 'Username': 'guest'})
