...
output_directory = "." 
model_name = "seckillcloud_3a35d54"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [infrastructural, local_logging, gateway], tagged_values = {'Gateway': 'Spring Cloud Gateway', 'Logging Technology': 'Lombok', 'Port': 8205})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [internal, encryption, csrf_disabled, local_logging, pre_authorized_endpoints], tagged_values = {'Logging Technology': 'Lombok', 'Pre-authorized Endpoints': "['/{id}/edit', '/updateUsing/{id}', '/{id}', '/deletes']", 'Port': 8070})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [message_broker, plaintext_credentials, infrastructural], tagged_values = {'Message Broker': 'RabbitMQ', 'Username': 'guest', 'Port': 5672, 'Password': 'guest'})
