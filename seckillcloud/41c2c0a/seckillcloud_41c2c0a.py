...
output_directory = "." 
model_name = "seckillcloud_41c2c0a"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [local_logging, infrastructural, gateway], tagged_values = {'Port': 8205, 'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [encryption, local_logging, csrf_disabled, internal, pre_authorized_endpoints], tagged_values = {'Port': 8070, 'Pre-authorized Endpoints': "['/{id}/edit', '/updateUsing/{id}', '/deletes', '/{id}']", 'Logging Technology': 'Lombok'})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [message_broker, plaintext_credentials, infrastructural], tagged_values = {'Password': 'guest', 'Username': 'guest', 'Message Broker': 'RabbitMQ', 'Port': 5672})
