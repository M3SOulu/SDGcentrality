...
output_directory = "." 
model_name = "seckillcloud_92dd1ed"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [local_logging, gateway, infrastructural], tagged_values = {'Logging Technology': 'Lombok', 'Port': 8205, 'Gateway': 'Spring Cloud Gateway'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [local_logging, encryption, pre_authorized_endpoints, csrf_disabled, internal], tagged_values = {'Logging Technology': 'Lombok', 'Port': 8070, 'Pre-authorized Endpoints': "['/deletes', '/{id}/edit', '/{id}', '/updateUsing/{id}']"})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [plaintext_credentials, infrastructural, message_broker], tagged_values = {'Username': 'guest', 'Port': 5672, 'Password': 'guest', 'Message Broker': 'RabbitMQ'})
