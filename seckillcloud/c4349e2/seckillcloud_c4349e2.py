...
output_directory = "." 
model_name = "seckillcloud_c4349e2"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [infrastructural, gateway, local_logging], tagged_values = {'Port': 8205, 'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [csrf_disabled, pre_authorized_endpoints, internal, encryption, local_logging], tagged_values = {'Pre-authorized Endpoints': "['/{id}/edit', '/{id}', '/updateUsing/{id}', '/deletes']", 'Port': 8070, 'Logging Technology': 'Lombok'})
...
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [infrastructural, message_broker, plaintext_credentials], tagged_values = {'Password': 'guest', 'Port': 5672, 'Message Broker': 'RabbitMQ', 'Username': 'guest'})
