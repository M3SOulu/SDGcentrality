...
output_directory = "." 
model_name = "seckillcloud_e2b0e1a"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [local_logging, gateway, infrastructural], tagged_values = {'Gateway': 'Spring Cloud Gateway', 'Port': 8205, 'Logging Technology': 'Lombok'})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [encryption, local_logging, csrf_disabled, internal, pre_authorized_endpoints], tagged_values = {'Pre-authorized Endpoints': "['/getMenus']", 'Logging Technology': 'Lombok', 'Port': 8070})
cloud_monitor = CClass(service, "cloud-monitor", stereotype_instances = [infrastructural, administration_server], tagged_values = {'Port': 8890, 'Administration Server': 'Spring Boot Admin'})
rabbitmq = CClass(service, "rabbitmq", stereotype_instances = [plaintext_credentials, message_broker, infrastructural], tagged_values = {'Username': 'guest', 'Port': 5672, 'Password': 'guest', 'Message Broker': 'RabbitMQ'})
mysql = CClass(service, "mysql", stereotype_instances = [internal], tagged_values = {'Port': 3305})
database_cloud_manage = CClass(external_component, "database-cloud-manage", stereotype_instances = [exitpoint, external_database, entrypoint], tagged_values = {'Database': 'MySQL'})
...
add_links({cloud_mission: rabbitmq}, stereotype_instances = [restful_http, plaintext_credentials_link, message_producer_rabbitmq], tagged_values = {'Routing Key': 'None', 'Producer Exchange': 'None'})
