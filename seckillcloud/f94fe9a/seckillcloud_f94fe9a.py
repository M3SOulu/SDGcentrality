...
output_directory = "." 
model_name = "seckillcloud_f94fe9a"
...
cloud_gateway = CClass(service, "cloud-gateway", stereotype_instances = [infrastructural, gateway, local_logging], tagged_values = {'Logging Technology': 'Lombok', 'Gateway': 'Spring Cloud Gateway', 'Port': 8205})
...
cloud_manage = CClass(service, "cloud-manage", stereotype_instances = [csrf_disabled, internal, pre_authorized_endpoints, encryption, local_logging], tagged_values = {'Logging Technology': 'Lombok', 'Pre-authorized Endpoints': "['/findAll']", 'Port': 8070})
...
broker = CClass(service, "broker", stereotype_instances = [internal], tagged_values = {'Port': 10909})
namesrv = CClass(service, "namesrv", stereotype_instances = [internal], tagged_values = {'Port': 9876})
mysql = CClass(service, "mysql", stereotype_instances = [internal], tagged_values = {'Port': 3305})
database_cloud_manage = CClass(external_component, "database-cloud-manage", stereotype_instances = [exitpoint, external_database, entrypoint], tagged_values = {'Port': 3306, 'Database': 'MySQL'})
...
add_links({broker: namesrv}, stereotype_instances = [restful_http])
add_links({dashboard: namesrv}, stereotype_instances = [restful_http])
