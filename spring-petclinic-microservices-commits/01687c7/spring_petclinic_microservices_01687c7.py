...
output_directory = "." 
model_name = "spring-petclinic-microservices_01687c7"
...
hystrix_dashboard = CClass(service, "hystrix-dashboard", stereotype_instances = [infrastructural, monitoring_dashboard], tagged_values = {'Monitoring Dashboard': 'Hystrix', 'Endpoints': "['/']"})
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [circuit_breaker, gateway, infrastructural, load_balancer], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix', 'Gateway': 'Spring Cloud Gateway', 'Endpoints': "['/api/gatewayowners/{ownerId}', '/api/gateway']"})
customers_service = CClass(service, "customers-service", stereotype_instances = [local_logging, internal], tagged_values = {'Logging Technology': 'Lombok', 'Endpoints': "['/owners', '/owners/{ownerId}']"})
...
add_links({api_gateway: user}, stereotype_instances = [restful_http, load_balanced_link, circuit_breaker_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: visits_service}, stereotype_instances = [restful_http, load_balanced_link, circuit_breaker_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: customers_service}, stereotype_instances = [restful_http, load_balanced_link, circuit_breaker_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: vets_service}, stereotype_instances = [restful_http, load_balanced_link, circuit_breaker_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: prometheus_server}, stereotype_instances = [restful_http, load_balanced_link, circuit_breaker_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
