...
output_directory = "." 
model_name = "spring-petclinic-microservices_20d87d9"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [load_balancer, circuit_breaker, gateway, infrastructural], tagged_values = {'Gateway': 'Spring Cloud Gateway', 'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud', 'Endpoints': "['/api/gatewayowners/{ownerId}', '/api/gateway']"})
...
tracing_server = CClass(service, "tracing-server", stereotype_instances = [tracing_server, infrastructural], tagged_values = {'Port': 9411, 'Tracing Server': 'Zipkin'})
...
add_links({api_gateway: user}, stereotype_instances = [circuit_breaker_link, restful_http, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
add_links({api_gateway: visits_service}, stereotype_instances = [circuit_breaker_link, restful_http, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
add_links({api_gateway: customers_service}, stereotype_instances = [circuit_breaker_link, restful_http, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
add_links({api_gateway: vets_service}, stereotype_instances = [circuit_breaker_link, restful_http, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
add_links({api_gateway: prometheus_server}, stereotype_instances = [circuit_breaker_link, restful_http, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
