...
output_directory = "." 
model_name = "spring-petclinic-microservices_4b18dc5"
...
admin_server = CClass(service, "admin-server", stereotype_instances = [infrastructural, administration_server], tagged_values = {'Port': 9090, 'Administration Server': 'Spring Boot Admin'})
vets_service = CClass(service, "vets-service", stereotype_instances = [internal], tagged_values = {'Endpoints': "['/vets']", 'Port': 8081})
api_gateway = CClass(service, "api-gateway", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Load Balancer': 'Spring Cloud', 'Port': 8081, 'Gateway': 'Zuul'})
customers_service = CClass(service, "customers-service", stereotype_instances = [internal, local_logging], tagged_values = {'Logging Technology': 'Lombok', 'Port': 8081, 'Endpoints': "['/owners/{ownerId}', '/owners']"})
