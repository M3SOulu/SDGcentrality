...
output_directory = "." 
model_name = "spring-petclinic-microservices_6dc4b29"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Load Balancer': 'Spring Cloud', 'Gateway': 'Zuul'})
spring_petclinic_ui = CClass(service, "spring-petclinic-ui", stereotype_instances = [internal])
customers_service = CClass(service, "customers-service", stereotype_instances = [internal], tagged_values = {'Endpoints': "['/owners', '/owners/{ownerId}']"})
