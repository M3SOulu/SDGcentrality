...
output_directory = "." 
model_name = "spring-petclinic-microservices_31cd262"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [gateway, load_balancer, infrastructural], tagged_values = {'Gateway': 'Zuul', 'Load Balancer': 'Spring Cloud'})
spring_petclinic_ui = CClass(service, "spring-petclinic-ui", stereotype_instances = [internal])
customers_service = CClass(service, "customers-service", stereotype_instances = [internal], tagged_values = {'Endpoints': "['/owners/{ownerId}', '/owners']"})
...
add_links({spring_petclinic_config_server: admin_server}, stereotype_instances = [restful_http])
