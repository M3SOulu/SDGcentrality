...
output_directory = "." 
model_name = "spring-petclinic-microservices_4124445"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [load_balancer, circuit_breaker, infrastructural, gateway], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud', 'Gateway': 'Zuul'})
customers_service = CClass(service, "customers-service", stereotype_instances = [internal, local_logging], tagged_values = {'Logging Technology': 'Lombok', 'Endpoints': "['/owners', '/owners/{ownerId}']"})
...
add_links({api_gateway: user}, stereotype_instances = [circuit_breaker_link, load_balanced_link, restful_http], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: prometheus_server}, stereotype_instances = [circuit_breaker_link, load_balanced_link, restful_http], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
