...
output_directory = "." 
model_name = "spring-petclinic-microservices_687e385"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Load Balancer': 'Spring Cloud', 'Gateway': 'Zuul', 'Port': 8081})
spring_petclinic_ui = CClass(service, "spring-petclinic-ui", stereotype_instances = [internal])
customers_service = CClass(service, "customers-service", stereotype_instances = [internal], tagged_values = {'Endpoints': "['/owners/{ownerId}', '/owners']", 'Port': 8081})
visits_service = CClass(service, "visits-service", stereotype_instances = [internal], tagged_values = {'Port': 8081})
