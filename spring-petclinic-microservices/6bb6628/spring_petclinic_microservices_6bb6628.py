...
output_directory = "." 
model_name = "spring-petclinic-microservices_6bb6628"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [load_balancer, gateway, infrastructural, circuit_breaker], tagged_values = {'Load Balancer': 'Ribbon', 'Circuit Breaker': 'Hystrix', 'Gateway': 'Zuul'})
customers_service = CClass(service, "customers-service", stereotype_instances = [internal, local_logging], tagged_values = {'Endpoints': "['/owners/{ownerId}', '/owners']", 'Logging Technology': 'Lombok'})
...
add_links({api_gateway: user}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
add_links({api_gateway: prometheus_server}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Load Balancer': 'Spring Cloud', 'Circuit Breaker': 'Hystrix'})
