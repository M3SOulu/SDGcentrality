...
output_directory = "." 
model_name = "spring-petclinic-microservices_744648a"
tracing_server = CClass(service, "tracing-server", stereotype_instances = [internal])
spring_petclinic_config_server = CClass(service, "spring-petclinic-config-server", stereotype_instances = [infrastructural, configuration_server], tagged_values = {'Port': 8888, 'Configuration Server': 'Spring Cloud Config'})
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [infrastructural, gateway, load_balancer], tagged_values = {'Load Balancer': 'Ribbon', 'Gateway': 'Zuul'})
spring_petclinic_ui = CClass(service, "spring-petclinic-ui", stereotype_instances = [internal])
customers_service = CClass(service, "customers-service", stereotype_instances = [internal, local_logging])
visits_service = CClass(service, "visits-service", stereotype_instances = [internal, local_logging])
