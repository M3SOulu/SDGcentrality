...
output_directory = "." 
model_name = "spring-petclinic-microservices_b501fdb"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [gateway, circuit_breaker, infrastructural, load_balancer], tagged_values = {'Circuit Breaker': 'Hystrix', 'Endpoints': "['/api/gatewayowners/{ownerId}', '/api/gateway']", 'Load Balancer': 'Spring Cloud', 'Gateway': 'Spring Cloud Gateway'})
...
add_links({api_gateway: user}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: customers_service}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: vets_service}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: visits_service}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
add_links({api_gateway: prometheus_server}, stereotype_instances = [restful_http, circuit_breaker_link, load_balanced_link], tagged_values = {'Circuit Breaker': 'Hystrix', 'Load Balancer': 'Spring Cloud'})
