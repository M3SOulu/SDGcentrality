...
output_directory = "." 
model_name = "spring-petclinic-microservices_b9a803c"
...
api_gateway = CClass(service, "api-gateway", stereotype_instances = [load_balancer, infrastructural, gateway], tagged_values = {'Port': 8081, 'Gateway': 'Zuul', 'Load Balancer': 'Spring Cloud'})
spring_petclinic_ui = CClass(service, "spring-petclinic-ui", stereotype_instances = [internal])
customers_service = CClass(service, "customers-service", stereotype_instances = [local_logging, internal], tagged_values = {'Port': 8081, 'Endpoints': "['/owners', '/owners/{ownerId}']", 'Logging Technology': 'Lombok'})
